...
def playerRating(player_id):
    ...
    # Goalkeepers Rating
    if (df['position_id'] == 1):
        weighted_rating = (saves_ratio * 100) + (clean_sheets_ratio * 200) + (penalties_saved_ratio * 15) + (df['pass_accuracy'] * 10) + (yellow_cards_ratio * -50) + (red_cards_ratio * -100)
   
    # Defenders Rating
    elif (df['position_id'] == 2):
        weighted_rating = (tackles_ratio * 50) + (clean_sheets_ratio * 30) + (total_passes_ratio * 3) + (df['pass_accuracy'] * 10) + (yellow_cards_ratio * -50) + (red_cards_ratio * -100)

    # Midfielders Rating
    elif (df['position_id'] == 3):
        weighted_rating = (goals_ratio * 90) + (assists_ratio * 110) + (total_passes_ratio * 7) + (df['pass_accuracy'] * 10) + (yellow_cards_ratio * -50) + (red_cards_ratio * -100)

    # Strikers/Wingers Rating
    elif (df['position_id'] == 4):
        weighted_rating = (goals_ratio * 110) + (assists_ratio * 100) + (total_passes_ratio * 5) + (df['pass_accuracy'] * 10) + (yellow_cards_ratio * -50) + (red_cards_ratio * -100)
